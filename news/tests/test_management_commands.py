...
class MockParserTests(TestCase):
    ...
    def _create_response(self, url, title, content, boring=False):
        template = '<p id="title">{title}</p><p id="content">{content}</p>'
        body = template.format(
            title=title.encode('ascii', 'xmlcharrefreplace').decode('ascii'),
            content=content.encode('ascii', 'xmlcharrefreplace').decode('ascii')
        )
    ...
    def test_parser(self):
        ...
        self.assertEqual(a1.version_set.count(), 2)
        self.assertEqual(a2.version_set.count(), 2)

        self.assertEqual(a1.version_set.exclude(pk=v1.pk).filter(
            article__url='http://www.mock.nl/mock_article1.html',
            byline='',
            diff_json='{"chars_added": 1, "chars_removed": 1}',
            title='\u1d90rticle 2'
        ).count(), 1)

        self.assertEqual(a2.version_set.exclude(pk=v2.pk).filter(
            article__url='http://www.mock.nl/mock_article2.html',
            byline='',
            diff_json='{"chars_added": 1, "chars_removed": 1}',
            title='\u1d90rticle 2'
    ...
    def test_parser_flapping(self):
        ...
        self._create_response(a1_url, '\u1d90rticle 1', "Body \u1d00 1\n")
        self._create_response(a2_url, '\u1d90rticle 1', "Body \u1d00 1\n")
        call_command('scraper')
        ...
        responses.reset()
        self._create_response(a1_url, '\u1d90rticle 2', "Body \u1d00 2\n")
        self._create_response(a2_url, '\u1d90rticle 2', "Body \u1d00 2\n")
        ...
        responses.reset()
        self._create_response(a1_url, '\u1d90rticle 3', "Body \u1d00 3\n")
        self._create_response(a2_url, '\u1d90rticle 3', "Body \u1d00 3\n")
        ...
        responses.reset()
        self._create_response(a1_url, '\u1d90rticle 1', "Body \u1d00 1\n")
        self._create_response(a2_url, '\u1d90rticle 4', "Body \u1d00 4\n")
        ...
        responses.reset()
        self._create_response(a1_url, '\u1d90rticle 5', "Body \u1d00 5\n")
        self._create_response(a2_url, '\u1d90rticle 5', "Body \u1d00 5\n")

        call_command('scraper')
        self.assertEqual(Version.objects.count(), 10)
        ...
        responses.reset()
        self._create_response(a1_url, '\u1d90rticle 1', "Body \u1d00 1\n")
        self._create_response(a2_url, '\u1d90rticle 6', "Body \u1d00 6\n")

        call_command('scraper')
        self.assertEqual(Version.objects.count(), 11)

        # Download version7 of article 1 and 2
        self._reset_update_time(a1, a2)

        responses.reset()
        self._create_response(a1_url, '\u1d90rticle 7', "Body \u1d00 7\n")
        self._create_response(a2_url, '\u1d90rticle 7', "Body \u1d00 7\n")

        call_command('scraper')
        self.assertEqual(Version.objects.count(), 13)
        ...
        responses.reset()
        self._create_response(a1_url, '\u1d90rticle 1', "Body \u1d00 1\n")
        self._create_response(a2_url, '\u1d90rticle 8', "Body \u1d00 8\n")

        call_command('scraper')
        self.assertEqual(Version.objects.count(), 14)
    ...
    def test_similarity_checking(self):
        ...
        ratio = levenshtein_seqratio([str(x) for x in version1.content_words_hashed], [str(x) for x in version2.content_words_hashed])
        ...
        self.assertEqual(SimilarArticle.objects.filter(**expected).count(), 1)
